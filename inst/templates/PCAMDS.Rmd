...
datasetScaled <- readRDS("datasetScaled.rds")
# umapResult <- readRDS("umapResult.rds")
...
### t-SNE
```{r tsne_plink, fig.width=12, fig.height=10, echo=FALSE, message=FALSE, warning=FALSE}
...
plot_umap <- ggplot(data = umapTable, aes(x = C1, y = C2, color = umapTable[,2], text = umapTable[,1])) +
  geom_point() +
  # geom_point_interactive(aes(x = wt, y = qsec, color = disp,
  #   tooltip = sample.id, data_id = sample.id)) +
  # geom_text(hjust=0, vjust=0, label=mds_tab$Sample, label.size=0.1) +
